...
bp_auth0 = Blueprint('auth0', __name__)
...
@bp_auth0.route('/login')
def login():
    callback_url = url_for('kubishi.auth0.callback_handling', _external=True)
    callback_url = callback_url.replace('127.0.0.1', 'localhost') # hack for auth0
    print(f"URL: ", url_for('kubishi.auth0.callback_handling', _external=True), flush=True)
    return auth0.authorize(callback=callback_url)

@bp_auth0.route('/callback')
def callback_handling():
    ...
    return redirect(url_for('kubishi.index'))

@bp_auth0.route('/logout')
def logout():
    session.clear()
    # logout from auth0.com too
    params = {'returnTo': url_for('kubishi.index', _external=True), 'client_id': AUTH0_CLIENT_ID}
...
@bp_auth0.route('/delete_account', methods=['POST'])
def delete_account():
    auth0_mgmt_api = Auth0(AUTH0_DOMAIN, get_management_api_token())
...
bp.register_blueprint(bp_auth0, url_prefix='/auth0')
