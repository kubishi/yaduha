...
class SentenceToEnglishTool(Tool[AgentResponse[str]], Generic[TSentenceType]):
    ...
    def _run(self, sentence: TSentenceType) -> AgentResponse:
        with inject_logs(tool="sentence_to_english"):
            example_messages = []
            for english, example_sentence in cast(
                list[tuple[str, TSentenceType]], sentence.get_examples()
            ):
                example_messages.append(
                    {
                        "role": "user",
                        "content": json.dumps(
                            example_sentence.model_dump_json(), ensure_ascii=False
                        ),
                    }
                )
                example_messages.append({"role": "assistant", "content": english})
