...
def main():
    path = os.path.join(thisdir, RESULTS_DIR, "sentences-translated")
    ...
    df["quality"] = "Bad"
    df.loc[
        (df["sim_simple"] > threshold) & (df["sim_comparator"] > threshold) & (df["sim_backwards"] > threshold),
        "quality",
    ] = "Good"
    df.loc[
        (df["sim_simple"] > threshold) & (df["sim_comparator"] < threshold) & (df["sim_backwards"] > threshold),
    ...
    fig.add_hrect(
        y0=sim_mean - 2 * sim_std,
        y1=sim_mean + 2 * sim_std,
        line_width=0,
        fillcolor="gray",
        opacity=0.3,
        layer="below",
    ...
    fig.add_hrect(
        y0=sim_mean - 3 * sim_std,
        y1=sim_mean + 3 * sim_std,
        line_width=0,
        fillcolor="gray",
        opacity=0.1,
        layer="below",
