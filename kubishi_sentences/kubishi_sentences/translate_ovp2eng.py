...
import openai
import pandas as pd
from kubishi_sentences.sentence_builder import (
    format_sentence,
    get_random_sentence,
    sentence_to_str,
)
...
def translate(
    ...
    object_suffix: Optional[str],
    model=None,
    res_callback: Optional[Callable[[ChatCompletion], None]] = None,
) -> str:
    if model is None:
    ...
    messages = [
        {
            "role": "system",
            "content": "You are an assistant for translating structured sentences into simple natural English sentences.",
        },
        *examples,
        {"role": "user", "content": json.dumps(structure)},
    ]
    res = openai.chat.completions.create(model=model, messages=messages, timeout=10, temperature=0.0)
    if res_callback:
        res_callback(res)
...
def main():
    ...
    evaluate_parser = subparsers.add_parser(
        "evaluate", help="Evaluate the translation of a number of randomly generated sentences"
    )
