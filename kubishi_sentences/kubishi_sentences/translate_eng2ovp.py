...
from kubishi_sentences.sentence_builder import NOUNS, Object, Subject, Verb
from kubishi_sentences.translate_ovp2eng import translate as translate_ovp_to_english
...
R_VERB_TENSES = {"present": "dü", "past": "ku", "future": "wei", "past_continuous": "ti", "present_continuous": "ti"}
R_SUBJECT_PRONOUNS = {
    "i": ["nüü"],
    "you": ["üü"],
    "he": ["uhu", "mahu"],
...
def translate_simple(sentence: Dict[str, str]) -> Tuple[Subject, Verb, Object]:
    ...
    if sentence.get("object"):
        ...
        verb_stem = R_INTRANSITIVE_VERBS.get(
            sentence["verb"], R_TRANSIITIVE_VERBS.get(sentence["verb"], f"[{sentence['verb']}]")
        )
...
def order_sentence(
    subject: Subject, verb: Verb, _object: Optional[Object] = None
) -> List[Union[Subject, Verb, Object]]:
...
def comparator_sentence(simple_sentence: Dict[str, str]) -> str:
    ...
    if (
        (simple_sentence.get("object") or "").strip()
        and simple_sentence["object"] not in R_NOUNS
        and simple_sentence["object"] not in R_OBJECT_PRONOUNS
    ):
        simple_sentence["object"] = "[OBJECT]"
    return simple_sentence


def translate_english_to_ovp(
    ...
    simple_sentences_nl = (
        ". ".join([make_sentence(sentence, model=model, res_callback=res_callback) for sentence in simple_sentences])
        + "."
    )
    comparator_sentence_nl = (
        ". ".join(
            [make_sentence(sentence, model=model, res_callback=res_callback) for sentence in comparator_sentences]
        )
        + "."
    )
            ...
            while True:
                try:
                    ...
                    simple_sentences_nl = (
                        ". ".join(
                            [
                                make_sentence(sentence, model=model, res_callback=res_callback)
...
def main():
    ...
    translate_parser.add_argument(
        "sentence", help="The English sentence to translate (if not provided, will enter interactive mode)", nargs="?"
    )
